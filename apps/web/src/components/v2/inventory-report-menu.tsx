...
export const inventoryReports = [
  { id: 'realtime', title: '实时库存查询表' }, { id: 'movements', title: '出入库明细表' }, { id: 'summary', title: '出入库汇总表' }, { id: 'transfer-detail', title: '机构间调拨明细表' }, { id: 'transfer-summary', title: '机构间调拨汇总表' },
]
export function InventoryReportMenu({ selected }: { selected: boolean }) {
  ...
  const open = () => { cancel(); const r = trigger.current?.getBoundingClientRect(); if (r) setTop(Math.max(12, Math.min(r.top, window.innerHeight - 400))) }
  useEffect(() => {
  ...
  return <>
    ...
    {top !== null && createPortal(<div ref={panel} id="inventory-report-flyout" onMouseEnter={cancel} onMouseLeave={close} onFocus={cancel} onBlur={close} style={{ top, left: 246 }} className="fixed z-[80] w-80 rounded-xl border border-border bg-white p-4 shadow-xl" aria-label="库存报表导航">
