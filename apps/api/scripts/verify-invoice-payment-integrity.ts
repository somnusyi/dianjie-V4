...
async function main() {
  ...
  const cashAccount = await prisma.cashAccount.create({
    data: {
      ...
      balance: 1000, accountNo: `6222${suffix.slice(-4)}`, cmbBindAccount: `LOCAL-${suffix}`, status: 'ACTIVE',
    },
  })
  const wechatAccount = await prisma.cashAccount.create({
    data: {
      tenantId: tenant.id, name: `付款验证微信账户-${suffix}`, type: 'WECHAT',
  ...
  const invoices = await Promise.all([
    ...
    makeInvoice(`INV-${suffix}-4`, 15),
    makeInvoice(`INV-${suffix}-5`, 10),
  ])
  const paymentIds: string[] = []
  const triggerName = `invoice_payment_guard_${suffix.replace(/[^a-z0-9]/gi, '_')}`
  const functionName = `${triggerName}_fn`
  const voucherTriggerName = `invoice_payment_voucher_${suffix.replace(/[^a-z0-9]/gi, '_')}`
  const voucherFunctionName = `${voucherTriggerName}_fn`
  ...
  try {
    ...
    const request60 = crypto.randomUUID()
    const competing = await Promise.all([
      api('/api/invoice-payments', financeToken, {
        method: 'POST', body: JSON.stringify({ invoiceId: invoices[0].id, amount: 60, paymentMethod: 'manual', requestId: request60 }),
      }),
      api('/api/invoice-payments', financeToken, {
        method: 'POST', body: JSON.stringify({ invoiceId: invoices[0].id, amount: 60, paymentMethod: 'manual', requestId: request60 }),
      }),
    ])
    assert.deepEqual(competing.map(result => result.status).sort(), [200, 201], '相同操作号并发应返回同一付款')
    assert.equal(new Set(competing.map(result => result.body.id)).size, 1)
    const payment60 = competing[0].body
    paymentIds.push(payment60.id)
    const conflictingRequest = await api('/api/invoice-payments', financeToken, {
      method: 'POST', body: JSON.stringify({ invoiceId: invoices[0].id, amount: 61, paymentMethod: 'manual', requestId: request60 }),
    })
    assert.equal(conflictingRequest.status, 409)
    const overReserved = await api('/api/invoice-payments', financeToken, {
      method: 'POST', body: JSON.stringify({ invoiceId: invoices[0].id, amount: 60, paymentMethod: 'manual', requestId: crypto.randomUUID() }),
    })
    assert.equal(overReserved.status, 409)
    const payment40Response = await api('/api/invoice-payments', financeToken, {
      method: 'POST', body: JSON.stringify({ invoiceId: invoices[0].id, amount: 40, paymentMethod: 'manual', requestId: crypto.randomUUID() }),
    })
    ...
    const duplicateConfirm = await Promise.all([
      api(`/api/invoice-payments/${payment60.id}/confirm`, financeToken, {
        method: 'PATCH', body: JSON.stringify({ status: 'SUCCESS', accountId: cashAccount.id, paidAt: '2026-07-16', bankTxNo: `TX-${suffix}-60` }),
      }),
      api(`/api/invoice-payments/${payment60.id}/confirm`, financeToken, {
        method: 'PATCH', body: JSON.stringify({ status: 'SUCCESS', accountId: cashAccount.id, paidAt: '2026-07-16', bankTxNo: `TX-${suffix}-60` }),
      }),
    ...
    const changedRetry = await api(`/api/invoice-payments/${payment60.id}/confirm`, financeToken, {
      method: 'PATCH', body: JSON.stringify({ status: 'SUCCESS', accountId: cashAccount.id, paidAt: '2026-07-15', bankTxNo: `TX-${suffix}-60` }),
    })
    assert.equal(changedRetry.status, 409)

    const payment30Response = await api('/api/invoice-payments', financeToken, {
      method: 'POST', body: JSON.stringify({ invoiceId: invoices[1].id, amount: 30, paymentMethod: 'manual', requestId: crypto.randomUUID() }),
    })
    ...
    const parallelDifferentInvoices = await Promise.all([
      api(`/api/invoice-payments/${payment40.id}/confirm`, financeToken, {
        method: 'PATCH', body: JSON.stringify({ status: 'SUCCESS', accountId: cashAccount.id, paidAt: '2026-07-16', bankTxNo: `TX-${suffix}-40` }),
      }),
      api(`/api/invoice-payments/${payment30.id}/confirm`, financeToken, {
        method: 'PATCH', body: JSON.stringify({ status: 'SUCCESS', accountId: cashAccount.id, paidAt: '2026-07-16', bankTxNo: `TX-${suffix}-30` }),
      }),
    ])
    ...
    const payment20Response = await api('/api/invoice-payments', financeToken, {
      method: 'POST', body: JSON.stringify({ invoiceId: invoices[2].id, amount: 20, paymentMethod: 'manual', requestId: crypto.randomUUID() }),
    })
    ...
    const missingReference = await api(`/api/invoice-payments/${payment20.id}/confirm`, financeToken, {
      method: 'PATCH', body: JSON.stringify({ status: 'SUCCESS', accountId: cashAccount.id, paidAt: '2026-07-16' }),
    })
    assert.equal(missingReference.status, 400)
    const wrongAccountType = await api(`/api/invoice-payments/${payment20.id}/confirm`, financeToken, {
      method: 'PATCH', body: JSON.stringify({ status: 'SUCCESS', accountId: wechatAccount.id, paidAt: '2026-07-16', bankTxNo: `TX-${suffix}-20` }),
    })
    assert.equal(wrongAccountType.status, 400)
    ...
    const failedAudit = await api(`/api/invoice-payments/${payment20.id}/confirm`, financeToken, {
      method: 'PATCH', body: JSON.stringify({ status: 'SUCCESS', accountId: cashAccount.id, paidAt: '2026-07-16', bankTxNo: `TX-${suffix}-20` }),
    })
    ...
    const payment15Response = await api('/api/invoice-payments', financeToken, {
      method: 'POST', body: JSON.stringify({ invoiceId: invoices[3].id, amount: 15, paymentMethod: 'manual', requestId: crypto.randomUUID() }),
    })
    assert.equal(payment15Response.status, 201, JSON.stringify(payment15Response.body))
    const payment15 = payment15Response.body
    paymentIds.push(payment15.id)
    const reusedReference = await api(`/api/invoice-payments/${payment15.id}/confirm`, financeToken, {
      method: 'PATCH', body: JSON.stringify({ status: 'SUCCESS', accountId: cashAccount.id, paidAt: '2026-07-16', bankTxNo: `TX-${suffix}-20` }),
    })
    assert.equal(reusedReference.status, 409)

    await prisma.$executeRawUnsafe(`
      CREATE OR REPLACE FUNCTION "${voucherFunctionName}"() RETURNS trigger AS $$
      BEGIN
        IF NEW."sourceType" = 'InvoicePayment' AND NEW."sourceId" = '${payment15.id}' THEN
          RAISE EXCEPTION 'forced invoice payment voucher failure';
        END IF;
        RETURN NEW;
      END;
      $$ LANGUAGE plpgsql
    `)
    await prisma.$executeRawUnsafe(`
      CREATE TRIGGER "${voucherTriggerName}"
      BEFORE INSERT ON "vouchers"
      FOR EACH ROW EXECUTE FUNCTION "${voucherFunctionName}"()
    `)
    const voucherFailed = await api(`/api/invoice-payments/${payment15.id}/confirm`, financeToken, {
      method: 'PATCH', body: JSON.stringify({ status: 'SUCCESS', accountId: cashAccount.id, paidAt: '2026-07-16', bankTxNo: `TX-${suffix}-15` }),
    })
    assert.equal(voucherFailed.status, 200, JSON.stringify(voucherFailed.body))
    assert.ok(voucherFailed.body.voucherWarning)
    assert.equal(Number((await prisma.cashAccount.findUniqueOrThrow({ where: { id: cashAccount.id } })).balance), 835)
    assert.equal(await prisma.cashTransaction.count({ where: { refType: 'InvoicePayment', refId: payment15.id } }), 1)
    await prisma.$executeRawUnsafe(`DROP TRIGGER IF EXISTS "${voucherTriggerName}" ON "vouchers"`)
    await prisma.$executeRawUnsafe(`DROP FUNCTION IF EXISTS "${voucherFunctionName}"()`)
    const voucherRecovered = await api(`/api/invoice-payments/${payment15.id}/confirm`, financeToken, {
      method: 'PATCH', body: JSON.stringify({ status: 'SUCCESS', accountId: cashAccount.id, paidAt: '2026-07-16', bankTxNo: `TX-${suffix}-15` }),
    })
    assert.equal(voucherRecovered.status, 200, JSON.stringify(voucherRecovered.body))
    assert.ok(voucherRecovered.body.voucherId)
    assert.equal(Number((await prisma.cashAccount.findUniqueOrThrow({ where: { id: cashAccount.id } })).balance), 835)
    assert.equal(await prisma.voucher.count({ where: { tenantId: tenant.id, sourceType: 'InvoicePayment', sourceId: payment15.id } }), 1)

    const payment10Response = await api('/api/invoice-payments', financeToken, {
      method: 'POST', body: JSON.stringify({ invoiceId: invoices[4].id, amount: 10, paymentMethod: 'manual', requestId: crypto.randomUUID() }),
    })
    assert.equal(payment10Response.status, 201)
    const payment10 = payment10Response.body
    paymentIds.push(payment10.id)
    assert.equal((await api(`/api/invoice-payments/${payment10.id}/confirm`, financeToken, {
      method: 'PATCH', body: JSON.stringify({ status: 'FAILED', failReason: '银行拒绝' }),
    })).status, 200)
    assert.equal((await api(`/api/invoice-payments/${payment10.id}/confirm`, financeToken, {
      method: 'PATCH', body: JSON.stringify({ status: 'FAILED', failReason: '其他原因' }),
    })).status, 409)

    console.log(JSON.stringify({
      ok: true,
      supplierBindingIsolation: true,
      requestIdIdempotentAndReservationBounded: true,
      duplicateConfirmationIdempotent: true,
      concurrentCashBalanceSerialized: true,
      auditFailureRollbackAndRetry: true,
      accountTypeAndReferenceGuarded: true,
      voucherFailureRetryWithoutDoubleDebit: true,
      terminalResultConflictGuarded: true,
    }))
  } finally {
    ...
    await prisma.$executeRawUnsafe(`DROP TRIGGER IF EXISTS "${voucherTriggerName}" ON "vouchers"`).catch(() => {})
    await prisma.$executeRawUnsafe(`DROP FUNCTION IF EXISTS "${voucherFunctionName}"()`).catch(() => {})
    await new Promise(resolve => setTimeout(resolve, 300))
    await prisma.voucher.deleteMany({
      where: { tenantId: tenant.id, sourceType: { in: ['Payment', 'InvoicePayment'] }, sourceId: { in: paymentIds } },
    })
    ...
    await prisma.cashAccount.delete({ where: { id: wechatAccount.id } })
