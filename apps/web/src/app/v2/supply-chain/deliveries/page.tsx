...
export default function InternalSupplyChainDeliveriesPage() {
  ...
  useEffect(() => {
    ...
    apiFetch<{ items: any[]; total: number; page: number; pageSize: number }>(
      `/api/deliveries${buildDeliveryQuery(filters)}`,
      { signal: controller.signal },
    )
      .then(data => {
        if (controller.signal.aborted) return
        setDeliveries((data.items || []).map(projectDeliveryRow))
  ...
  const filterActive = hasActiveDeliveryFilters(draftFilters)

  function updateDraftFilters(changes: Partial<DeliveryFilters>) {
    ...
    setDraftFilters(current => resetDeliveryFilterPage(current, changes))
  ...
  function clearFilters() {
    const cleared = { ...DEFAULT_DELIVERY_FILTERS }
      ...
      <header className="mx-auto flex max-w-[1440px] flex-col gap-3 border-b border-border pb-5 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <div className="mb-2 flex items-center gap-2">
            ...
            <span className="text-caption text-gray3">配送单查询 · 跨店</span>
          </div>
          <h1 className="text-h1">配送单查询</h1>
      ...
      <main className="mx-auto max-w-[1440px]">
        ...
        <div className="flex flex-wrap items-end gap-3 py-4">
          <FilterInput
            ...
            placeholder="配送单号 / 订货单号 / 门店 / 商品名称 / 编码 / 规格"
          />
          <FilterSelect
            label="门店"
          ...
          <FilterSelect
            ...
            onChange={value => updateDraftFilters({ status: value as DeliveryFilters['status'] })}
          >
        ...
        {!deliveries && !error && <div className="space-y-2">{[1, 2, 3].map(i => <SkeletonCard key={i} />)}</div>}

        {deliveries && deliveries.length === 0 && !loading && (
          <EmptyState
            icon="🚚"
            title={filterActive ? '没有匹配的配送记录' : '暂无配送记录'}
            hint={filterActive ? '尝试调整筛选条件' : '配送单将在供应商发货后生成'}
          />
        )}
        ...
        {total > 0 && (
          <div className="flex items-center justify-between py-4 text-caption text-gray2">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => setFilters(current => keepDeliveryFiltersForPage(current, filters.page - 1))}
