...
async function main() {
  ...
  try {
    ...
    for (const path of ['/api/revenue', '/api/revenue/summary']) {
      assert.equal((await api(path, unboundManagerToken)).status, 403, path)
    }
    assert.equal((await api('/api/revenue', unboundManagerToken, {
      method: 'POST', body: JSON.stringify({ date: '2026-07-22', amount: 1 }),
    })).status, 403)
    const paymentRequestCount = await prisma.document.count({
      where: { tenantId: tenant.id, type: 'PAYMENT_REQUEST', initiatorId: unboundManager.id },
    })
    assert.equal((await api('/api/payment-requests', unboundManagerToken, {
      method: 'POST',
      body: JSON.stringify({ payeeName: '未绑定门店测试收款方', amount: 1, usage: 'repair' }),
    })).status, 403)
    ...
    const supplierSchedules = await api('/api/schedules', supplierToken)
    assert.equal(supplierSchedules.status, 200, JSON.stringify(supplierSchedules.body))
    const runSchedules = supplierSchedules.body.filter((schedule: any) => receiptIds.includes(schedule.receiptId))
    assert.equal(runSchedules.length, 2, '供应商应能查看本轮两笔到账计划')
    for (const schedule of runSchedules) {
      for (const field of [
        'bankRawResponse',
        'retryCount',
        'paymentId',
        ...
        assert.equal(Object.hasOwn(schedule, field), false, `供应商账期列表不应返回 ${field}`)
      }
    }
    ...
    assert.equal((await api('/api/deliveries?productCode=未声明字段', supplierToken)).status, 400, '配送单应拒绝未知查询字段')
    ...
    const rankedSku = firstSkuRank.body.top.find((item: any) => item.productId === product.id)
    assert.equal(rankedSku?.orders, 1, '同一订货单拆分为两次入库时，SKU 成交单数必须去重')
