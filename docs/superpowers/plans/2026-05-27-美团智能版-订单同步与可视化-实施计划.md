...
```bash
cd ~/Desktop/dianjie-V4/dianjie-V4/packages/db
pnpm prisma migrate dev --name add_meituan_tables --create-only
```

Expected: 生成 `prisma/migrations/<timestamp>_add_meituan_tables/migration.sql`，含 7 张 CREATE TABLE 语句。

- [ ] **Step 3：审 migration SQL**

```bash
cat ~/Desktop/dianjie-V4/dianjie-V4/packages/db/prisma/migrations/*_add_meituan_tables/migration.sql | head -100
```

Expected: 看到 `CREATE TABLE "meituan_api_call_logs"` 等 7 个 CREATE TABLE，**不含任何 ALTER 现有表**。如果有 ALTER 现有表 → 删除 migration 文件并核对 schema 改动范围。

- [ ] **Step 4：本地应用 migration**

```bash
pnpm prisma migrate dev
```

Expected: "Applied successfully" + `prisma generate` 也自动跑。

- [ ] **Step 5：验证 Prisma client 有新 model**

```bash
cd ~/Desktop/dianjie-V4/dianjie-V4
node -e "const {prisma}=require('./packages/db/dist/index.js'); console.log(Object.keys(prisma).filter(k=>k.startsWith('m') || k.startsWith('M')))"
```

或者直接看 generated client：

```bash
grep "^  meituanApiCallLog\|^  mtOrder\b\|^  meituanAuthToken" ~/Desktop/dianjie-V4/dianjie-V4/node_modules/.prisma/client/index.d.ts | head -10
```

Expected: 看到 `meituanApiCallLog`, `meituanSyncCursor`, `mtOrder`, `mtOrderItem`, `mtOrderPayment`, `mtRefundOrder`, `meituanAuthToken` 7 个 model getter。
