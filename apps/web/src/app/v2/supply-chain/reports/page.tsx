      ...
      {!id ? <p>从“库存报表”菜单选择一张报表，继续查看。</p> : <>
        <form className={styles.card} onSubmit={e => { e.preventDefault(); apply() }}>
          <div className={styles.filters}>
            ...
            {input('keyword', '物品名称 / 编码')}{id === 'realtime' && input('category', '物品类别')}
            {id !== 'realtime' && <>{input('start', transfer ? '调拨开始日期' : '开始日期', 'date')}{input('end', '结束日期', 'date')}</>}

          </div>
          {advanced && <div className={`${styles.filters} ${styles.advancedFilters}`}>{!transfer && <label>机构名称<input value="总部" readOnly /></label>}{id !== 'realtime' && input('category', '物品类别')}            {['movements', 'summary'].includes(id) && <label>出入库类型<select value={filters.type} onChange={e => setFilters(f => ({ ...f, type: e.target.value }))}>{['', '期初建账', '手工入库', '采购入库', '出库', '库存调整', '报损', '冲销'].map(v => <option key={v} value={v}>{v || '全部'}</option>)}</select></label>}
            {['movements', 'transfer-detail'].includes(id) && input('doc', '单据号')}{input('unit', '单位')}{id === 'realtime' && input('conversion', '单位换算')}{id === 'movements' && <>{input('upstream', '上游单据号')}{input('upstreamType', '上游单据类型')}{input('reason', '原因类型')}{input('counterparty', '对方机构')}<label>调整单标识<select value={filters.adjustment} onChange={e => setFilters(f => ({ ...f, adjustment: e.target.value }))}><option value="">全部</option><option>是</option><option>否</option></select></label></>}{result?.columns.filter(c => c.kind === 'number').map(c => <label key={c.key}>{c.group} {c.label}<span className={styles.range}><input aria-label={`${c.group || ''}${c.label}最小值`} type="number" step="any" placeholder="最小值" value={ranges[c.key]?.min ?? ''} onChange={e => setRanges(r => ({ ...r, [c.key]: { ...r[c.key], min: e.target.value === '' ? undefined : Number(e.target.value) } }))} /><input aria-label={`${c.group || ''}${c.label}最大值`} type="number" step="any" placeholder="最大值" value={ranges[c.key]?.max ?? ''} onChange={e => setRanges(r => ({ ...r, [c.key]: { ...r[c.key], max: e.target.value === '' ? undefined : Number(e.target.value) } }))} /></span></label>)}</div>}
