...
async function main() {
  ...
  const admin = await prisma.user.upsert({
    ...
    update: {},
    create: { tenantId: tenant.id, name: '系统管理员', email: 'admin@dianjie.com', password: hash('dj123456'), role: Role.ADMIN },
  })
  const finance = await prisma.user.upsert({
    where: { tenantId_email: { tenantId: tenant.id, email: 'finance@dianjie.com' } },
    update: {},
    create: { tenantId: tenant.id, name: '李慧芳', email: 'finance@dianjie.com', password: hash('dj123456'), role: Role.FINANCE },
  })
  ...
  const mgr1 = await prisma.user.upsert({
    ...
    update: {},
    create: { tenantId: tenant.id, name: '王建国', email: 'manager1@dianjie.com', password: hash('dj123456'), role: Role.MANAGER, storeId: stores[0].id },
  })
  const mgr2 = await prisma.user.upsert({
    where: { tenantId_email: { tenantId: tenant.id, email: 'manager2@dianjie.com' } },
    update: {},
    create: { tenantId: tenant.id, name: '张晓燕', email: 'manager2@dianjie.com', password: hash('dj123456'), role: Role.MANAGER, storeId: stores[1].id },
  })
  ...
  await prisma.user.upsert({
    ...
    update: { supplierId: sup1.id },
    create: {
      tenantId: tenant.id, name: '赵总（楚雄菌业）',
      email: 'supplier1@dianjie.com', password: hash('dj123456'),
  ...
  await prisma.user.upsert({
    ...
    update: { supplierId: sup2.id },
    create: {
      tenantId: tenant.id, name: '扎西（迪庆蔬菜）',
      email: 'supplier2@dianjie.com', password: hash('dj123456'),
      role: 'SUPPLIER_STAFF' as Role,
      supplierId: sup2.id,
    },
  })
  ...
  // 场景5：已收货但有差异（用于报损下拉选项）
  const po5 = await prisma.purchaseOrder.upsert({
    where: { tenantId_no: { tenantId: tenant.id, no: 'PO202402002' } },
    update: {},
    create: {
      tenantId: tenant.id, no: 'PO202402002',
      storeId: stores[0].id, supplierId: sup1.id,
      expectedDate: d('2026-02-25'), totalAmount: 6960,
      status: 'RECEIVED',
      shippedAt: d('2026-02-25T08:00:00'), shippedById: admin.id,
      receivedAt: d('2026-02-25T14:00:00'),
      createdById: mgr1.id,
      items: { create: [
        { productId: p1.id, quantity: 15, unitPrice: 380, amount: 5700, receivedQty: 13 },
        { productId: p3.id, quantity: 5, unitPrice: 260, amount: 1300, receivedQty: 4 },
      ]},
    },
  })

  console.log('✓ 采购订单创建完成（5笔）')
  ...
  console.log('\n✅ 测试数据初始化完成！')
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
  console.log('登录账号（统一密码 dj123456）：')
  console.log('  管理员    admin@dianjie.com')
  console.log('  财务      finance@dianjie.com')
  console.log('  翠湖店长  manager1@dianjie.com')
  console.log('  大理店长  manager2@dianjie.com')
  console.log('  供应商1   supplier1@dianjie.com')
  console.log('  供应商2   supplier2@dianjie.com')
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')
  console.log('测试场景：')
