...
2026-09-21 17:47（Asia/Shanghai），用户确认 UI 后完成本地复核：

- API `/health` 返回 `status=ok, db=ok`；PostgreSQL、Redis 容器正常，Redis 返回 PONG。
- Prisma 检测到 92 个迁移，数据库结构为最新，无待执行迁移。
- 使用真实账号登录，从 Web 3210 的同源代理和 API 4444 直连分别查询全部 5 张报表，记录内容及总数一致，每张当前均为 1 条本地测试记录。
- 对全部 5 张报表导出 Excel，解包检查行数及各数值列（包括空值），与列表完整结果一致。
- 只读 SQL 核对：余额表=全部流水增量=实时库存=期间汇总期末，数量 100kg、金额 1000 元。
- 只读 SQL 核对：已发货/已收货的调拨明细合计=两张调拨报表，数量 5kg、调出金额 50 元、调入金额 60 元。
- 真实账号权限：供应链和财务可读取全部 5 张报表；供应商全部返回 403；未登录返回 401。
- 重新运行新增 PostgreSQL 集成测试，6 组全部通过，覆盖写入持久化、状态转换、幂等、跨租户隔离及导出。
- 浏览器刷新后正确显示库存汇总，未捕获到页面错误。

本轮未发现连接或对账问题，未修改业务代码。验证范围是本地隔离测试库和当前测试数据，不代表远程 UAT 或生产环境已部署/验收。门店调拨登记仍不自动增减门店库存；缺少税率依据的明细金额仍显示“—”。
