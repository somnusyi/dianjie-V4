 ...
 * 内部供应链 · 收货查询（桌面端 · 只读）
 *
 * 消费 /api/receipts 分页列表；筛选全部由服务端执行。
 * 不提供任何写操作按钮；不展示财务/应付/银行字段。
 */
'use client'
...
export default function InternalSupplyChainReceiptsPage() {
  ...
  useEffect(() => {
    ...
    apiFetch<{ items: any[]; total: number; page: number; pageSize: number }>(
      `/api/receipts${buildReceiptQuery(filters)}`,
    )
      .then(data => {
        if (sequence !== requestSequence.current) return
        setReceipts((data.items || []).map(projectReceiptRow))
  ...
  const totalPages = useMemo(() => receiptTotalPages(total, filters.pageSize), [total, filters.pageSize])
  const range = useMemo(() => receiptPaginationRange(filters.page, filters.pageSize, total), [filters.page, filters.pageSize, total])
  const filterActive = hasActiveReceiptFilters(draftFilters)

  function updateDraftFilters(changes: Partial<ReceiptFilters>) {
    ...
    setDraftFilters(current => resetReceiptFilterPage(current, changes))
    setDateError(validateReceiptDateRange(nextDateFrom, nextDateTo))
  }

  function applyFilters() {
    const validationError = validateReceiptDateRange(draftFilters.dateFrom, draftFilters.dateTo)
  ...
  function clearFilters() {
    const cleared = { ...DEFAULT_RECEIPT_FILTERS }
      ...
      <header className="mx-auto flex max-w-[1440px] flex-col gap-3 border-b border-border pb-5 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <div className="mb-2 flex items-center gap-2">
            ...
            <span className="text-caption text-gray3">收货查询 · 跨店</span>
          </div>
          <h1 className="text-h1">收货查询</h1>
          <p className="mt-1 text-caption text-gray2">
            {receipts ? `${total} 条收货记录` : '加载中…'}
          </p>
        </div>
      ...
      <main className="mx-auto max-w-[1440px]">
        <div className="flex flex-wrap items-end gap-3 py-4">
          <FilterInput
            ...
            placeholder="收货单号 / 门店 / 商品名称 / 编码 / 规格"
          />
          <FilterSelect
            label="门店"
        ...
        {!receipts && !error && <div className="space-y-2">{[1, 2, 3].map(i => <SkeletonCard key={i} />)}</div>}

        {receipts && receipts.length === 0 && !loading && (
          <EmptyState
            icon="📋"
            title={filterActive ? '没有匹配的收货记录' : '暂无收货记录'}
            hint={filterActive ? '尝试调整筛选条件' : '收货记录将在门店确认入库后出现'}
          />
        )}
              ...
              <table className="w-full text-left text-caption">
                <thead className="bg-bg text-gray3">
                  <tr>
                    ...
                    <th className="px-4 py-3">业务到货日</th>
        ...
        {total > 0 && (
          <div className="flex items-center justify-between py-4 text-caption text-gray2">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => setFilters(current => keepReceiptFiltersForPage(current, filters.page - 1))}
